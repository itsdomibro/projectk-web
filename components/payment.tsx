      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center animate-in zoom-in duration-300">
          ...
          <div className="bg-green-50 rounded-xl p-4 mb-6 border border-green-100">
            ...
            <div className="text-xl font-bold text-green-700">{formatCurrency(transaction.totalAmount)}</div>
            <div className="text-xs text-gray-400 mt-1 font-mono">#{transaction.code}</div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 flex flex-col items-center text-center h-full">
            ...
            <div className="bg-white p-4 border-2 border-blue-100 rounded-3xl mb-8 shadow-sm">
              <QRCode
                value={`PAYMENT:${transaction.transactionId}:${transaction.totalAmount}`}
                size={220}
                level="H"
            ...
            <div className="space-y-1 mb-8 w-full">
              ...
              <p className="text-3xl font-bold text-blue-600">{formatCurrency(transaction.totalAmount)}</p>
              <p className="text-xs text-gray-400 mt-2 font-mono select-all">Kode: {transaction.code}</p>
            </div>
            ...
            <div className="w-full space-y-3 mt-auto">
              ...
              >
                {paymentStatus === 'processing' ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Simulasi Pembayaran Berhasil'}
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto max-h-[400px] pr-2 space-y-4 mb-6 custom-scrollbar">
              {/* PERBAIKAN DISINI: Mapping dari details (bukan items) */}
              {transaction.details?.map((item: any, idx: number) => (
                <div key={idx} className="flex justify-between items-start text-sm group border-b border-gray-50 pb-3 last:border-0">
                  <div>
                    ...
                    <div className="text-gray-500 mt-1 text-xs">{formatCurrency(item.price)} x {item.quantity}</div>
                  </div>
            ...
            <div className="flex justify-between items-center pt-4 mt-4 border-t border-gray-100 mb-6">
              ...
              <span className="text-gray-900 font-extrabold text-xl">{formatCurrency(transaction.totalAmount)}</span>
            </div>
          </div>
