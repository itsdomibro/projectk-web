...
export default function OwnerDashboardPage() {
  ...
  const handleGenerateAISummary = async () => {
    ...
    const summary = `# 📊 Analisis Bisnis (${
      filterOptions.find((f) => f.value === dateFilter)?.label
    })\n\n**Ringkasan:**\nTotal pendapatan mencapai **${formatCurrency(
      totalRevenue
    )}** dari **${totalTransactions}** transaksi.\n\n**Top Produk:**\n${topProducts
      .map((p, i) => `${i + 1}. ${p.name} (${p.quantity} terjual)`)
      .join(
        "\n"
        ...
        {aiSummary ? (
          ...
          <div className="text-center py-12 border-2 border-dashed border-purple-100 rounded-xl bg-white/50">
            ...
            <p className="text-gray-500">
              Klik tombol di atas untuk mendapatkan analisis otomatis.
            </p>
          </div>
        )}
      </div>
