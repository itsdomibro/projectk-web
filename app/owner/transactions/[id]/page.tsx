...
import { ImageWithFallback } from '@/components/ImageWithFallback'; // Pastikan path import ini sesuai struktur Anda
import type { Transaction } from '@/types';

// Helper untuk unwrap params di Next.js 15+ (jika pakai versi terbaru)
// atau langsung gunakan props jika versi lama. 
// Disini saya pakai React.use() yang aman untuk Next.js 13-15 client components
export default function TransactionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (id) {
      // Decode ID jika perlu, lalu cari transaksi
      const found = transactions.find(t => t.id === decodeURIComponent(id));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Kartu Utama Detail Transaksi */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden print:shadow-none print:border-0">
          
          {/* Header Invoice */}
          <div className="bg-gray-900 text-white p-8 print:bg-white print:text-black print:p-0 print:border-b print:mb-6">
            <div className="flex flex-col md:flex-row justify-between md:items-start gap-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center print:hidden">
              ...
              <div className="text-right">
                ...
                <p className="text-gray-400 font-mono text-sm print:text-gray-600 mb-4">#{transaction.id}</p>
                <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                  transaction.status === 'completed' 
                    ? 'bg-green-500 text-white print:text-black print:border print:border-black' 
          ...
          <div className="p-8 border-b border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <p className="text-gray-900 font-medium">Umum (Guest)</p>
              <p className="text-gray-500 text-sm">Walk-in Customer</p>
            </div>
            <div>
              <h3 className="text-xs font-bold text-gray-400 uppercase mb-2 flex items-center gap-1">
                <CreditCard className="w-3 h-3" /> Metode Bayar
              </h3>
              <p className="text-gray-900 font-medium">QRIS / Tunai</p>
              <p className="text-gray-500 text-sm">Otomatis</p>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {transaction.items.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-4 py-4 text-center text-gray-900 font-medium hidden md:table-cell">
                        {item.quantity}
                      </td>
          ...
          {/* Footer Card */}
          <div className="bg-gray-50 p-6 border-t border-gray-200 print:bg-white print:border-t-2 print:border-black print:mt-8">
            <p className="text-center text-gray-500 text-sm">
              Terima kasih telah berbelanja di Toko POS. <br className="hidden print:block"/>
              Simpan bukti pembayaran ini sebagai jaminan garansi (jika berlaku).
            </p>
          </div>
        </div>
